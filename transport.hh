...
#ifndef _DL_TRANSPORT_
#define _DL_TRANSPORT_
    ...
    public:
      ...
      void run() {
        ...
        try {
          ...
          while(dl::read_pb(request, input)) {
            _handlerR(context, request, response);
            dl::write_pb(response, output);
            output.flush();
            request.Clear();
            response.Clear();
          }
...
public:
  ...
  void send(const Request& request) {
    ...
    dl::write_pb(request, stream);
  }

  void receive(Response& response) {
    Poco::Net::SocketStream stream(_stream);
    dl::read_pb(response, stream);
  }
